  ...
  const handleProcess = async () => {
    ...
    <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-lg bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-2xl overflow-hidden animate-[fadeInUp_0.25s_ease-out]">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 dark:border-slate-800">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 bg-purple-50 dark:bg-purple-600/20 border border-purple-100 dark:border-purple-600/30 rounded-xl flex items-center justify-center">
              <Upload size={18} className="text-purple-605 dark:text-purple-400" />
            </div>
            <div>
              <h3 className="font-black text-slate-900 dark:text-white text-base">Bulk Invite Users</h3>
              <p className="text-xs text-slate-505 dark:text-slate-400 font-medium">Upload Excel or CSV to invite multiple users</p>
            </div>
          </div>
          <button onClick={onClose} className="text-slate-400 hover:text-slate-600 dark:text-slate-500 dark:hover:text-white transition-colors p-1"><X size={20} /></button>
        </div>
          ...
          {!result ? (
            <>
                ...
                {file ? (
                  <div className="flex flex-col items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">{(file.size / 1024).toFixed(1)} KB</p>
                    </div>
                    <button
                      onClick={e => { e.stopPropagation(); setFile(null); }}
                      className="text-xs text-red-600 dark:text-red-400 hover:text-red-500 dark:hover:text-red-300 font-semibold"
                    >
                      Remove
                    </button>
                  </div>
                  ...
                  <div className="flex flex-col items-center gap-3">
                    ...
                    <div>
                      <p className="font-bold text-slate-705 dark:text-slate-300 text-sm">Drop file here or click to upload</p>
              ...
              <button
                ...
                className="flex items-center gap-2 text-sm text-blue-600 dark:text-blue-400 hover:text-blue-500 dark:hover:text-blue-300 font-semibold mt-4 transition-colors"
              >
            ...
            <div>
              ...
              {result.failed.length > 0 && (
                <div>
                  <p className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-3">Error Details</p>
                  <div className="max-h-48 overflow-y-auto rounded-xl border border-slate-200 dark:border-slate-700 divide-y divide-slate-100 dark:divide-slate-800">
                    {result.failed.map((f) => (
                      <div key={f.row} className="flex items-start gap-3 px-4 py-3">
                        <span className="text-xs font-bold text-slate-400 dark:text-slate-505 w-8 shrink-0">#{f.row}</span>
        ...
        {/* Footer */}
        <div className="flex items-center justify-end gap-3 px-6 py-4 border-t border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/30">
          <button onClick={onClose} className="px-5 py-2.5 border border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-350 hover:bg-slate-100 dark:hover:bg-slate-850 rounded-xl text-sm font-bold transition-all">
            {result ? 'Close' : 'Cancel'}
          </button>
          {!result && (
            <button
