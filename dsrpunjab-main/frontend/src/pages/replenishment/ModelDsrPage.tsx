...
async function downloadCompiledModelDsr(report: ModelDsrReport, sections: SectionDef[], project: ProjectDetail | null) {
  ...
  pages.forEach((reportPage, index) => {
    ...
    reportPage.drawText(headerText.slice(0, 110), {
      x: 38,
      y: height - 34,
      size: 8,
      font: regular,
    ...
    reportPage.drawText(footerText.slice(0, 95), {
      x: 38,
      y: 26,
      size: 7.5,
      font: regular,
  ...
  if (showGeneratedPreface) {
    pages.push(
      <div key="preface" className="mb-5 flex w-full justify-center">
        <GeneratedSection chapter={{ name: "Preface", summary: frontData.preface! }} pageNumber={pages.length + 1} district={district} headerText={headerText} footerText={footerText} />
      </div>
    );
  }
  ...
  if (showAcknowledgement) {
    pages.push(
      <div key="ack" className="mb-5 flex w-full justify-center">
        <GeneratedSection chapter={{ name: "Acknowledgement", summary: frontData.acknowledgement! }} pageNumber={pages.length + 1} district={district} headerText={headerText} footerText={footerText} />
      </div>
    );
  }
  ...
  if (checkedSet.has("chapters") && content.chapters.length > 0) {
    pages.push(
      <div key="title-chapters" className="mb-5 flex w-full justify-center">
        <SectionTitlePage title="Chapters" pageNumber={pages.length + 1} district={district} headerText={headerText} footerText={footerText} />
      </div>
    );
    content.chapters.forEach((chap, i) => {
      pages.push(
        <div key={`chapter-${i}`} className="mb-5 flex w-full justify-center">
          <GeneratedSection chapter={chap} pageNumber={pages.length + 1} district={district} headerText={headerText} footerText={footerText} />
        </div>
      );
    });
  }
  ...
  if (checkedSet.has("plates") && content.plates.length > 0) {
    pages.push(
      <div key="title-plates" className="mb-5 flex w-full justify-center">
        <SectionTitlePage title="Plates and Maps" pageNumber={pages.length + 1} district={district} headerText={headerText} footerText={footerText} />
      </div>
    );
    content.plates.forEach((plate, i) => {
      pages.push(
        <div key={`plate-${i}`} className="mb-5 flex w-full justify-center">
          <GeneratedSection chapter={{ name: plate.name, summary: plate.summary || "" }} pageNumber={pages.length + 1} district={district} headerText={headerText} footerText={footerText} />
        </div>
      );
    });
  }
  ...
  uploadedPreviews.forEach((upload, i) => {
    pages.push(
      <div key={`upload-${upload.id}-${i}`} className="mb-5 flex w-full justify-center">
        <UploadedSection upload={{ id: upload.id, title: upload.title, name: upload.title, url: upload.url }} pageNumber={pages.length + 1} district={district} headerText={headerText} footerText={footerText} />
      </div>
    );
  });
