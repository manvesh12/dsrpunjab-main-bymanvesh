...
import { SectionTitlePage, GeneratedSection, UploadedSection } from "../reports/ReportPreviewPage";
...
interface ModelDsrReport {
  ...
  sectionOrder: string[];
  pageRanges: Record<string, { start: number; end?: number }>;
}
...
function uploadRangeKey(upload: ProjectUpload) {
  return `${upload.sectionId}::${upload.name}::${upload.url}`;
}

function normalizedPageRange(report: ModelDsrReport, upload: ProjectUpload, totalPages?: number) {
  const saved = report.pageRanges?.[uploadRangeKey(upload)];
  const maximum = totalPages === undefined ? Number.MAX_SAFE_INTEGER : Math.max(totalPages, 1);
  const start = Math.min(Math.max(Math.trunc(saved?.start || 1), 1), maximum);
  const requestedEnd = saved?.end ?? totalPages;
  const end = requestedEnd === undefined
    ? undefined
    : Math.min(Math.max(Math.trunc(requestedEnd), start), maximum);
  return { start, end };
}
...
function selectedModelUploads(report: ModelDsrReport, project: ProjectDetail | null): ProjectUpload[] {
  ...
  const push = (sectionId: string, title: string, file?: { name: string; url?: string } | null) => {
    if (file?.url) uploads.push({ sectionId, title, name: file.name, url: file.url });
  };
  const frontMap = [
  ...
  frontMap.forEach(([sectionId, uploadKey, title, projectFile]) => {
    ...
    if (modelUrl) uploads.push({ sectionId, title, name: `${title}.pdf`, url: modelUrl });
    else push(sectionId, title, projectFile);
  });
  if (checked.has("chapters")) content.chapters.forEach((chapter) => push("chapters", chapter.name, chapter.file));
  if (checked.has("plates")) content.plates.forEach((plate) => push("plates", plate.name, plate.url ? { name: plate.fileName || plate.name, url: plate.url } : null));
  report.customSections.forEach((section) => {
    if (!checked.has(section.id)) return;
    (report.customPdfs?.[section.id] || []).forEach((url, index) => uploads.push({ sectionId: section.id, title: section.name, name: `${section.name}-${index + 1}.pdf`, url }));
  });
  (project?.files || []).forEach((file) => {
    const key = file.objectKey.toLowerCase();
    const matchingAnnexure = report.sections.find((sectionId) => {
    ...
    if (matchingAnnexure) uploads.push({ sectionId: matchingAnnexure, title: file.fileName, name: file.fileName, url: uploadsApi.getDownloadUrl(file.annexureId) });
  });
...
function loadReports(projectId: string): ModelDsrReport[] {
  try {
    const reports = JSON.parse(localStorage.getItem(reportsKey(projectId)) || "[]") as ModelDsrReport[];
    return reports.map((report) => ({
      ...report,
      headerText: report.headerText || report.name || "Model DSR Report",
      footerText: report.footerText || "Generated by DSR Portal",
      sectionOverrides: report.sectionOverrides || {},
      pageRanges: report.pageRanges || {},
    }));
  } catch {
    return [];
  }
    ...
    return data
      ...
      .map((s) => {
        ...
        return {
          ...
          headerText: state.headerText || s.title || "Model DSR Report",
          footerText: state.footerText || "Generated by DSR Portal",
    ...
    const res = await apiClient.post<{ id: string; createdAt: string }>(
      `/projects/${projectId}/replenishment`,
      {
        ...
        reportState: {
          type: "model_dsr",
          headerText: name,
          footerText: "Generated by DSR Portal",
          sectionOverrides: {},
          sections: [],
          frontMatterPdfs: {},
          customPdfs: {},
...
async function downloadCompiledModelDsr(report: ModelDsrReport, sections: SectionDef[], project: ProjectDetail | null) {
  ...
  pageSectionIds.push("title-page");
  ...
  for (const upload of selectedModelUploads(report, project)) {
    try {
      ...
      await appendUploadedDocument(document, { name: upload.name, url: upload.url }, { pageRange: normalizedPageRange(report, upload) });
      const appendedPageCount = document.getPageCount() - startPage;
      pageSectionIds.push(...Array.from({ length: appendedPageCount }, () => upload.sectionId));
    } catch (error) {
  ...
  const headerText = report.headerText?.trim() || report.name || "Model DSR Report";
  const footerText = report.footerText?.trim() || "Generated by DSR Portal";
  const pages = document.getPages();
  pages.forEach((reportPage, index) => {
    const { width, height } = reportPage.getSize();
    const frameColor = rgb(0.07, 0.22, 0.38);
    const mutedColor = rgb(0.28, 0.35, 0.43);
    const pageLabel = `Page ${index + 1} of ${pages.length}`;
    const override = report.sectionOverrides?.[pageSectionIds[index]];
    const pageHeaderText = override?.headerText?.trim() || headerText;
    const pageFooterText = override?.footerText?.trim() || footerText;
    ...
    reportPage.drawLine({
      start: { x: 34, y: height - 40 },
      end: { x: width - 34, y: height - 40 },
      thickness: 0.6,
      color: frameColor,
    });
    reportPage.drawText(pageHeaderText.slice(0, 110), {
      x: 38,
      y: height - 34,
      size: 8,
      font: regular,
    ...
    reportPage.drawText(pageFooterText.slice(0, 95), {
      x: 38,
      y: 26,
      size: 7.5,
      font: regular,
    ...
    reportPage.drawText(pageLabel, {
      x: width - 38 - regular.widthOfTextAtSize(pageLabel, 7.5),
      y: 26,
      size: 7.5,
      font: regular,
      color: mutedColor,
...
function ReportEditor({
  ...
  const [selectedFrameSection, setSelectedFrameSection] = useState("title-page");
  ...
  const frameSections = [
    ...
    ...orderedSections.flatMap((section) => [
      ...
      ...(section.subsections || []).map((subsection) => ({ id: subsection.id, name: subsection.name })),
    ]),
  ];
  const selectedFrameLabel = frameSections.find((section) => section.id === selectedFrameSection)?.name || "Selected section";
  const selectedFrameOverride = report.sectionOverrides?.[selectedFrameSection] || {};
    ...
    onChange({
      ...
      sectionOverrides: {
        ...
        [selectedFrameSection]: {
          ...report.sectionOverrides?.[selectedFrameSection],
          [field]: value,
        },
      },
    });
      ...
      <section className="border-b border-slate-200 bg-slate-50 px-5 py-4">
        ...
        <label className="text-xs font-semibold text-slate-600">
          ...
          <input
            ...
            placeholder={report.name || "Model DSR Report"}
        ...
        <label className="text-xs font-semibold text-slate-600">
          ...
          <input
            ...
            placeholder="Generated by DSR Portal"
            className="mt-1.5 h-9 w-full rounded-lg border border-slate-300 bg-white px-3 text-sm font-medium normal-case tracking-normal text-slate-800 outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100"
          />
        </label>
        <label className="text-xs font-semibold text-slate-600">
          ...
          <select
            value={selectedFrameSection}
            onChange={(event) => setSelectedFrameSection(event.target.value)}
            className="mt-1.5 h-9 w-full rounded-lg border border-slate-300 bg-white px-3 text-sm font-medium text-slate-800 outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100"
          >
            {frameSections.map((section) => <option key={section.id} value={section.id}>{section.name}</option>)}
          </select>
        </label>
        </div>
        <div className="mt-3 grid gap-3 md:grid-cols-2">
          <label className="text-xs text-slate-600">
            {selectedFrameLabel} header
            <input
              value={selectedFrameOverride.headerText || ""}
              onChange={(event) => updateFrameOverride("headerText", event.target.value)}
              placeholder="Uses default header if empty"
              className="mt-1.5 h-9 w-full rounded-lg border border-slate-300 bg-white px-3 text-sm font-medium text-slate-800 outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100"
            />
          </label>
          <label className="text-xs text-slate-600">
            {selectedFrameLabel} footer
            <input
              value={selectedFrameOverride.footerText || ""}
              onChange={(event) => updateFrameOverride("footerText", event.target.value)}
              placeholder="Uses default footer if empty"
              className="mt-1.5 h-9 w-full rounded-lg border border-slate-300 bg-white px-3 text-sm font-medium text-slate-800 outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100"
            />
          </label>
        </div>
...
function DraggableSectionItem({
  ...
  const { projectId = "default" } = useParams();
  const sectionUploads = selectedModelUploads(report, project).filter((upload) => upload.sectionId === section.id);

  const updatePageRange = (upload: ProjectUpload, field: "start" | "end", rawValue: string) => {
    const value = Math.max(1, Math.trunc(Number(rawValue) || 1));
    const key = uploadRangeKey(upload);
    const current = report.pageRanges?.[key] || { start: 1 };
    const next = field === "start"
      ? { ...current, start: value, end: current.end && current.end < value ? value : current.end }
      : { ...current, end: Math.max(value, current.start || 1) };
    onChange({ ...report, pageRanges: { ...(report.pageRanges || {}), [key]: next } });
  };
  ...
  return (
    ...
    >
      ...
      {sectionUploads.length > 0 && (
        <div className="mx-3 mb-3 space-y-2 rounded-lg border border-blue-100 bg-white p-2.5">
          <p className="text-[10px] font-bold uppercase tracking-wide text-blue-700">PDF page selection</p>
          {sectionUploads.map((upload) => {
            const range = report.pageRanges?.[uploadRangeKey(upload)] || { start: 1 };
            return (
              <div key={uploadRangeKey(upload)} className="grid grid-cols-[minmax(0,1fr)_64px_64px] items-end gap-2">
                <div className="min-w-0">
                  <p className="truncate text-[11px] font-semibold text-slate-700" title={upload.name}>{upload.title || upload.name}</p>
                  <p className="truncate text-[9px] text-slate-400">{upload.name}</p>
                </div>
                <label className="text-[9px] font-semibold text-slate-500">From
                  <input type="number" min={1} value={range.start || 1} onChange={(event) => updatePageRange(upload, "start", event.target.value)} className="mt-0.5 h-7 w-full rounded border border-slate-300 px-2 text-xs text-slate-800" />
                </label>
                <label className="text-[9px] font-semibold text-slate-500">To
                  <input type="number" min={range.start || 1} value={range.end ?? ""} placeholder="Last" onChange={(event) => {
                    if (!event.target.value) {
                      const key = uploadRangeKey(upload);
                      const nextRanges = { ...(report.pageRanges || {}) };
                      nextRanges[key] = { start: range.start || 1 };
                      onChange({ ...report, pageRanges: nextRanges });
                    } else updatePageRange(upload, "end", event.target.value);
                  }} className="mt-0.5 h-7 w-full rounded border border-slate-300 px-2 text-xs text-slate-800" />
                </label>
              </div>
            );
          })}
          <p className="text-[9px] text-slate-400">Leave “To” empty to include through the last page.</p>
        </div>
      )}
...
function PreviewPanel({
  ...
  const district = project?.district || "Punjab";
  const year = project?.year || "2025-26";
  const headerText = report.headerText?.trim() || report.name || "Model DSR Report";
  const footerText = report.footerText?.trim() || "Generated by DSR Portal";
  const frameFor = (sectionId: string) => {
    const override = report.sectionOverrides?.[sectionId];
    return {
      headerText: override?.headerText?.trim() || headerText,
      footerText: override?.footerText?.trim() || footerText,
    };
  };
  const titleFrame = frameFor("title-page");
  const prefaceFrame = frameFor("fm-pref");
  const acknowledgementFrame = frameFor("fm-ack");
  const chaptersFrame = frameFor("chapters");
  const platesFrame = frameFor("plates");
  const safeHeaderText = escapeHtml(titleFrame.headerText);
  const safeFooterText = escapeHtml(titleFrame.footerText);
  const safeReportName = escapeHtml(report.name || "Model DSR Report");
...
<style>
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .page { position: relative; background: #fff; max-width: 700px; margin: 0 auto; padding: 82px 64px 72px; border: 1px solid #111827; box-shadow: 0 4px 24px rgba(0,0,0,0.10); min-height: 900px; }
  .running-header { position: absolute; left: 48px; right: 48px; top: 28px; border-bottom: 1px solid #17324d; padding-bottom: 7px; font: italic 10px Georgia, serif; color: #334155; }
  .running-footer { position: absolute; left: 48px; right: 48px; bottom: 26px; display: flex; justify-content: space-between; border-top: 1px solid #94a3b8; padding-top: 7px; font-size: 9px; color: #64748b; }
...
<body>
<div class="page">
  <div class="running-header">${safeHeaderText}</div>
  ...
  <div class="report-title">
    <h3>${safeReportName}</h3>
    <p>${safeDistrict} | ${safeYear}</p>
  </div>
  ...
  <div class="running-footer"><span>${safeFooterText}</span><span>Page 1</span></div>
</div>
</body>
</html>`;
  ...
  pages.push(
    <section key="title-page" className="mb-5 shadow-sm border border-slate-200 bg-white">
      <iframe srcDoc={previewHtml} className="w-full h-[900px] border-none" title="Report Title Page" />
    </section>
  );

  if (showGeneratedPreface) {
    pages.push(
      <div key="preface" className="mb-5 flex w-full justify-center">
        <GeneratedSection chapter={{ name: "Preface", summary: frontData.preface! }} pageNumber={pages.length + 1} district={district} headerText={prefaceFrame.headerText} footerText={prefaceFrame.footerText} />
      </div>
    );
  }
  ...
  if (showAcknowledgement) {
    pages.push(
      <div key="ack" className="mb-5 flex w-full justify-center">
        <GeneratedSection chapter={{ name: "Acknowledgement", summary: frontData.acknowledgement! }} pageNumber={pages.length + 1} district={district} headerText={acknowledgementFrame.headerText} footerText={acknowledgementFrame.footerText} />
      </div>
    );
  }

  if (checkedSet.has("chapters") && content.chapters.length > 0) {
    pages.push(
      <div key="title-chapters" className="mb-5 flex w-full justify-center">
        <SectionTitlePage title="Chapters" pageNumber={pages.length + 1} district={district} headerText={chaptersFrame.headerText} footerText={chaptersFrame.footerText} />
      </div>
    );
    content.chapters.forEach((chap, i) => {
      pages.push(
        <div key={`chapter-${i}`} className="mb-5 flex w-full justify-center">
          <GeneratedSection chapter={chap} pageNumber={pages.length + 1} district={district} headerText={chaptersFrame.headerText} footerText={chaptersFrame.footerText} />
        </div>
      );
    });
  }

  if (checkedSet.has("plates") && content.plates.length > 0) {
    pages.push(
      <div key="title-plates" className="mb-5 flex w-full justify-center">
        <SectionTitlePage title="Plates and Maps" pageNumber={pages.length + 1} district={district} headerText={platesFrame.headerText} footerText={platesFrame.footerText} />
      </div>
    );
    content.plates.forEach((plate, i) => {
      pages.push(
        <div key={`plate-${i}`} className="mb-5 flex w-full justify-center">
          <GeneratedSection chapter={{ name: plate.name, summary: plate.summary || "" }} pageNumber={pages.length + 1} district={district} headerText={platesFrame.headerText} footerText={platesFrame.footerText} />
        </div>
      );
    });
  }

  if (uploadedPreviews.length === 0 && !checkedSet.has("chapters") && !checkedSet.has("plates") && !showGeneratedPreface && !showAcknowledgement) {
    pages.push(
      <div key="empty" className="mt-5 flex min-h-[300px] w-full items-center justify-center rounded-xl border border-dashed border-slate-300 bg-white p-8 text-center text-sm font-semibold text-slate-500">
  ...
  uploadedPreviews.forEach((upload, i) => {
    ...
    const range = normalizedPageRange(report, upload, sourcePageCount);
    for (let sourcePageNumber = range.start; sourcePageNumber <= (range.end ?? sourcePageCount); sourcePageNumber += 1) {
      pages.push(
