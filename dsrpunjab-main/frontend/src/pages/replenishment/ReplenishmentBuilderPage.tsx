...
function photoWidth(value?: number) {
  const parsed = Number(value || 100);
  if (!Number.isFinite(parsed)) return 100;
  return Math.min(100, Math.max(35, Math.round(parsed)));
}
...
function renderPhotoHtml(file: ReplenishmentFile, captionText?: string, width = 100, inGroup = false) {
  const caption = safe(captionText?.trim() || file.fileName);
  const style = `width:${photoWidth(width)}%;max-width:100%;${inGroup ? "" : "margin-left:auto;margin-right:auto;"}`;
  if (file.downloadUrl) {
    return `<figure class="inserted-photo" style="${style}"><img src="${safe(file.downloadUrl)}" alt="${caption}"><figcaption>${caption}</figcaption></figure>`;
  }
  return `<figure class="inserted-photo inserted-photo-placeholder" style="${style}"><figcaption>Photo: ${caption}</figcaption></figure>`;
}

function renderPhotoGroupHtml(groupFiles: ReplenishmentFile[], photoCaptions: Record<string, string>, photoWidths: Record<string, number>, columns: number) {
  const safeColumns = Math.min(3, Math.max(1, Math.round(columns || 2)));
  return `<div class="photo-layout" style="grid-template-columns:repeat(${safeColumns},minmax(0,1fr))">${groupFiles.map((file) => renderPhotoHtml(file, photoCaptions[file.id], photoWidths[file.id], true)).join("")}</div>`;
}

function renderContentWithInserts(content: string, tables: CalculatedGridTable[], files: ReplenishmentFile[], photoCaptions: Record<string, string> = {}, photoWidths: Record<string, number> = {}) {
  ...
  return content.split(/(\[\[(?:TABLE|PHOTO|PHOTO_GROUP):[^\]]+\]\])/g).map((part) => {
    ...
    const photoGroupToken = part.match(/^\[\[PHOTO_GROUP:([^|\]]+)(?:\|cols=(\d+))?\]\]$/);
    if (photoGroupToken) {
      const groupFiles = photoGroupToken[1].split(",").map((id) => photoMap.get(id.trim())).filter(Boolean) as ReplenishmentFile[];
      return groupFiles.length ? renderPhotoGroupHtml(groupFiles, photoCaptions, photoWidths, Number(photoGroupToken[2] || 2)) : `<p><b>Missing photo group</b></p>`;
    }
    const photoToken = part.match(/^\[\[PHOTO:([^\]]+)\]\]$/);
    if (photoToken) {
      const match = photoMap.get(photoToken[1]);
      return match ? renderPhotoHtml(match, photoCaptions[match.id], photoWidths[match.id]) : `<p><b>Missing photo:</b> ${safe(photoToken[1])}</p>`;
    }
...
function buildFormalReportHtml(state: BuilderState, _study: ReplenishmentStudy | null, files: ReplenishmentFile[]) {
  ...
  const body = sections.map((section, index) => `<section class="chapter">${frameFor(section.id)}<h2>${index + 1}.0 ${cleanTitle(section.title)}</h2><div class="narrative">${renderContentWithInserts(section.content, calculationTables, files, state.photoCaptions, state.photoWidths)}</div>${section.id === "calculation" ? calculationHtml : ""}</section>`).join("");
  const evidenceRows = evidenceRequirements.map((requirement, index) => {
    const matches = files.filter((file) => file.sectionId === requirement.id);
    return `<tr><td>${index + 1}</td><td>${safe(requirement.title)}</td><td>${requirement.required ? "Required" : "Optional"}</td><td>${matches.length ? matches.map((file) => safe(file.fileName)).join("<br>") : "Not uploaded"}</td><td>${matches.length ? "Attached" : "Pending"}</td></tr>`;
  }).join("");
...
function PhotoFileEditor({
  ...
  return (
    <div className={`rounded-lg border p-2 ${selected ? "border-[#12396b] bg-blue-50" : "border-transparent bg-slate-50"}`}>
      <div className="flex items-center gap-2">
        {onToggle && <input type="checkbox" checked={Boolean(selected)} onChange={onToggle} className="h-4 w-4 accent-[#12396b]" />}
        <button onClick={onDownload} className="flex min-w-0 flex-1 items-center justify-between gap-2 rounded px-1 py-1 text-left text-xs hover:bg-blue-50">
          <span className="truncate font-medium text-[#12396b]">{file.fileName}</span>
          <span className="shrink-0 text-slate-400">{fileSize(file.sizeBytes)}</span>
        </button>
      </div>
      <label className="mt-2 block">
        <span className="mb-1 block text-[11px] font-bold text-slate-500">Photo caption</span>
        <input value={caption} onChange={(event) => onCaptionChange(event.target.value)} placeholder="Text shown below this photo in final report" className="w-full rounded border border-slate-300 bg-white px-2.5 py-2 text-xs outline-none focus:border-[#12396b]"/>
      </label>
      <label className="mt-2 block">
        <span className="mb-1 flex items-center justify-between text-[11px] font-bold text-slate-500"><span>Photo size</span><span>{photoWidth(width)}%</span></span>
        <input type="range" min={35} max={100} step={5} value={photoWidth(width)} onChange={(event) => onWidthChange(Number(event.target.value))} className="w-full accent-[#12396b]" />
      </label>
      <div className="mt-2 rounded-lg border border-slate-200 bg-white p-2">
        <div className="mb-1 flex items-center justify-between text-[11px] font-bold text-slate-500">
          <span>Live resize preview</span>
          <span>{photoWidth(width)}%</span>
        </div>
        <div className="flex h-36 items-center justify-center overflow-hidden rounded bg-slate-50">
          {previewSrc ? (
            <img
              ...
              style={{ width: `${photoWidth(width)}%` }}
            />
          ) : (
            <span className="text-xs text-slate-400">Preview loading...</span>
          )}
        </div>
      </div>
    </div>
...
export default function ReplenishmentBuilderPage() {
  ...
  const [state, setState] = useState<BuilderState>({ type: "replenishment_builder_v2", schemaVersion: 2, details: defaultDetails, sectionOverrides: {}, sections: defaultSections, grids: [defaultGrid()], gridTables: [defaultGridTable()], photoCaptions: {}, photoWidths: {}, inherited: {}, importedKeys: [] });
  ...
  const [photoLayoutColumns, setPhotoLayoutColumns] = useState(2);
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const active = studies.find((item) => item.reportState?.type === "replenishment_builder_v2") || studies[0] || await replenishmentApi.create(projectId, { title: defaultDetails.reportTitle, reportState: { type: "replenishment_builder_v2", schemaVersion: 2, details: defaultDetails, sections: defaultSections, grids: firstTable.grids, gridTables: [firstTable], photoCaptions: {}, photoWidths: {}, importedKeys: [] } });
      setStudy(active);
  ...
  const updatePhotoWidth = (fileId: string, width: number) => setState((current) => ({ ...current, photoWidths: { ...(current.photoWidths || {}), [fileId]: photoWidth(width) } }));
  const toggleSelectedPhoto = (fileId: string) => setSelectedPhotoIds((current) => current.includes(fileId) ? current.filter((id) => id !== fileId) : [...current, fileId]);
  const insertSelectedPhotos = () => {
    if (!selectedPhotoIds.length) {
      toast.error("Select photos first");
      return;
    }
    const token = selectedPhotoIds.length === 1 ? `[[PHOTO:${selectedPhotoIds[0]}]]` : `[[PHOTO_GROUP:${selectedPhotoIds.join(",")}|cols=${photoLayoutColumns}]]`;
    insertIntoActiveSection(token);
  };
